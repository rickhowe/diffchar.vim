		...
		if 0 < tu[1].l && 0 < tu[2].l
			for k in [1, 2]
				...
				let tu[k].u = split(t:DChar.opt.iw == 0 ? tu[k].t :
								\substitute(tu[k].t, (t:DChar.opt.iw == 1) ?
									\'\s\+' : '\s\+$', '', 'g'), t:DChar.upa)
					...
					if t:DChar.opt.iw == 1
						if tu[k].t =~ '\s\+'
							let pc = filter(range(1, len(tu[k].t)),
											\'tu[k].t[v:val - 1] !~ "\\s"')
							call map(tu[k].c, '[v:val[0],
								\[pc[v:val[1][0] - 1], pc[v:val[1][1] - 1]]]')
						endif
					endif
...
function! s:ResetDiffChar(...) abort
	...
	let dl = {'1': [], '2': []}
	for n in (0 < reset) ? range(len(t:DChar.dfl[ak])) :
					\filter(map(copy(a:1), 'index(t:DChar.dfl[ak], v:val)'),
															\'v:val != -1')
		for k in [1, 2]
			...
			if !empty(dc)
				for bn in values(dc.bnr)
					if index([cb, nb[0]], bn) != -1
						call s:EchoWarning('Both or either selected buffer
							\ already highlighted in tab page ' . tn . '!')
						return -1
					endif
				endfor
			endif
	...
	if 0 < pv
		...
		if pv == 3 || pv == 4
			let t:DChar.dpv.pw = s:VF.PopupWindow ? 0 :
												\s:VF.FloatingWindow ? {} : -1
		endif
	endif
	let t:DChar.mid = {'1': {}, '2': {}}
	let t:DChar.hlc = {'1': {}, '2': {}}
	let t:DChar.cks = {'1': {}, '2': {}}
	let du = t:DChar.opt.ut
	if du == 'Char'
		let t:DChar.upa = '\zs'
	elseif du == 'Word2'
		...
		while 1
			...
			if id == synIDtrans(id)
				...
				if !empty(bg) && bg != fg && bg != bd &&
					...
					\'standout'], 'synIDattr(id, v:val)'), '!empty(v:val)'))
					let hl[bg] = nm
				endif
			endif
			let id += 1
	...
	elseif dc == 100
		...
		while 1
			...
			if index(values(s:DCharHL), nm) == -1 && id == synIDtrans(id) &&
					...
					\'italic', 'standout'], '!empty(synIDattr(id, v:val))'))
...
function! s:SetDiffCharHL() abort
	...
	for [fs, ts, th, ta] in [['C', 'C', 'dcDiffChange', ''],
		...
		call execute(['highlight clear ' . th, 'highlight ' . th . ' ' .
...
function! s:GetDiffCharOptions() abort
	let opt = {}
	let opt.ut = get(t:, 'DiffUnit', g:DiffUnit)
	let opt.cl = get(t:, 'DiffColors', g:DiffColors)
	let opt.pv = get(t:, 'DiffPairVisible', g:DiffPairVisible)
	let do = split(&diffopt, ',')
	let opt.ic = (index(do, 'icase') != -1)
	let opt.iw = (index(do, 'iwhiteall') != -1) ? 1 :
											\(index(do, 'iwhite') != -1) ? 2 :
									\(index(do, 'iwhiteeol') != -1) ? 3 : 0
	let opt.ih = (index(do, 'indent-heuristic') != -1)
	return opt
endfunction
...
function! s:ToggleDiffCharEvent(on) abort
	...
	for td in tv
		for k in [1, 2]
			...
			let ac += [[s:VF.WinClosed ? 'WinClosed' : 'BufWinLeave', bl,
													\'s:WinClosedDiffChar()']]
			if td.dfp != 0
				let ac += [[s:VF.WinScrolled ? 'WinScrolled' :
						\'CursorMoved', bl, 's:ScrollDiffChar(' . k . ')']]
			endif
			if 0 < td.dpv.pv
	...
	let ac += [['TabEnter', '*', 's:ToggleDiffHL(exists("t:DChar"))']]
	...
	for ed in split(a:es, '[+-]\+\zs', 1)[: -2]
		...
		for k in [0, 1]
			...
			if 0 < qq[k]
				...
				let hh = [
					\ll[k] - ((0 < pp[k]) ? len(strcharpart(a:uu[k][pp[k] - 1],
...
function! s:FocusDiffLines(init) abort
		...
		if a:init
			for k in [1, 2]
				call win_execute(t:DChar.wid[k],
						\'let dfl[k] = s:GetDiffLines(1, t:DChar.lcc[k].ll)')
			endfor
			return dfl
		else
			return t:DChar.dfl
	...
	for k in [1, 2]
		...
		endwhile
		call win_execute(t:DChar.wid[k],
					\'let dfl[k] = s:GetDiffLines(tx + 1, tl - 1) + dfl[k] +
	...
	for k in [1, 2]
		let tb[k] = {'tl': 1, 'ti': -k, 'tc': -k,
								\'bl': t:DChar.lcc[k].ll, 'bi': -k, 'bc': -k}
		if !empty(dfl[k])
			if !init
...
function! s:RedrawDiffChar(key) abort
	...
	if t:DChar.lcc[a:key].cn != lcc.cn
		...
		let pfl = t:DChar.dfl
		if pfl[ak] == cfl[ak]
			let ddl = filter(copy(pfl[ak]),
										\'pfl[bk][v:key] != cfl[bk][v:key] ||
											\get(t:DChar.cks[ak], v:val) !=
									\s:ChecksumStr(getbufline(t:DChar.bnr[ak],
														\cfl[ak][v:key])[0])')
			let adl = ddl
			let sdl = []
		else
...
function! s:ToggleDiffCharPair(on) abort
	if t:DChar.dpv.pv == 3 || t:DChar.dpv.pv == 4
		...
		if s:VF.PopupWindow
			if oe == [1, 1]
				let t:DChar.dpv.pw = popup_create('', {'hidden': 1,
...
def! s:GetDiffUnitPos(uu: list<any>, es: string): list<any>
	...
	for ed in split(es, '[+-]\+\zs', 1)[: -2]
		...
		for k in [0, 1]
			if 0 < qe
				[ll[k], pp[k]] +=
							[len(join(uu[k][pp[k] : pp[k] + qe - 1], '')), qe]
			endif
			var hh: list<number>
			if 0 < qq[k]
				hh = [ll[k]]
				...
				hh = [
					ll[k] - ((0 < pp[k]) ? len(strcharpart(uu[k][pp[k] - 1],
		...
		for [l, ec] in items(lec[k])
			...
			for [hh, cc] in items(hc)
				for ic in range(0, len(cc) - 1, 8)
					t:DChar.mid[k][ln] += [matchaddpos(hh, cc[ic : ic + 7],
										-3, -1, {'window': t:DChar.wid[k]})]
				endfor
			endfor
		endfor
	endfor
